...
export interface Pairing { id:string; codeHash:string; qrPayload:string; nonce:string; device:DeviceInfo; status:PairingStatus; userId?:string; userName?:string; attempts:number; expiresAt:Date; createdAt:Date; used:boolean }
...
export class MemoryStore { users = new Map<string,User>(); pairings = new Map<string,Pairing>(); sessions = new Map<string,DeviceSession>(); transfers = new Map<string,Transfer>(); }
